/// Dart FFI binding to the native the reading frontend reading frontend
...
/// token/tone tensors. One [JaReadingFrontend] is single-threaded — create one per
...
class JaReadingFrontend {
  JaReadingFrontend._(this._handle, this._kana, this._free, this._destroy);
  ...
  factory JaReadingFrontend.load(String dictDir, {String? libraryPath}) {
    ...
    return JaReadingFrontend._(handle, kana, free, destroy);
  }

  /// Katakana reading of [text] (e.g. "今日は元気ですか" → "キョーワゲンキデスカ").
  String kana(String text) {
    if (_disposed) throw StateError('JaReadingFrontend used after dispose');
