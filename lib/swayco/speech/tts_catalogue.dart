...
final List<TtsModelSpec> _specs = <TtsModelSpec>[
  // Premium (Piper `high`) where the release has one; every other language
  // stays on its best available tier (`medium`, or lower where that is the
  // ceiling). en and de are the only launch-adjacent languages with a `high`
  // bundle — de is the SAME speaker (Thorsten) at higher quality, en swaps amy
  // (medium) for lessac, the canonical high-quality US voice. fr has no `high`
  // (siwis-medium is its ceiling); ja is premium already via its melo engine.
  _piper('en_US-lessac-high', ['en'], mb: 110),
  _piper('fr_FR-siwis-medium', ['fr']),
  _piper('es_ES-davefx-medium', ['es']),
  _piper('pt_BR-faber-medium', ['pt']),
  _piper('it_IT-paola-medium', ['it']),
  _piper('de_DE-thorsten-high', ['de'], mb: 110),
