...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    // SWAYCO PATCH: targets without code-asset support (the Flutter WEB build)
    // provide no `input.config.code`, so the unconditional `input.config.code`
    // dereference below throws a null-check and breaks `flutter build web`.
    // `buildCodeAssets` is the canonical guard (see the `hooks` package docs);
    // the upstream 0.1.9 hook omits it. There is nothing native to build for
    // web — return before touching the code config.
    if (!input.config.buildCodeAssets) {
      logger.info('llm_llamacpp: no code assets for this target — skipping');
      return;
    }
